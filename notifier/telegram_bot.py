...
def authorized_only(func):
   ...
   async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
       ...
       if not allowed_id:
           logger.warning("TELEGRAM_CHAT_ID non configuré — commande rejetée")
           return
...
async def init_telegram() -> bool:
   ...
   if not config.TELEGRAM_CHAT_ID:
       logger.warning("TELEGRAM_CHAT_ID non configuré — les commandes seront rejetées")
   try:
...
async def _fetch_current_prices(open_trades: list[dict]) -> dict[str, float]:
   """
   Fetch les prix live depuis DexScreener.
   Une requête par token avec filtrage par chain pour éviter les confusions.
   """
   prices = {}
   if not open_trades:
       return prices
           ...
           for trade in open_trades:
               ...
               try:
                   async with session.get(
                       ...
                       if resp.status != 200:
                           logger.warning(
                               "DexScreener HTTP %s pour %s",
                               resp.status, trade.get("token_symbol")
                           )
                           continue
                       data = await resp.json()
                       pairs = data.get("pairs", [])
                       if not pairs:
                           logger.debug("Aucun pair trouvé pour %s", addr)
                           continue

                       # Filtrer par chain, fallback toutes chains si rien
                       chain_pairs = [
                           p for p in pairs
                       ...
                       # Prendre le pair avec la meilleure liquidité
                       best = max(
                           chain_pairs,
                           key=lambda p: float(
                   ...
                   logger.warning(
                       "Erreur fetch prix %s: %s",
                       trade.get("token_symbol"), e
                   )
                   continue

   except Exception as e:
       logger.warning("Erreur session fetch prix: %s", e)
...
async def cmd_positions(update: Update, context: ContextTypes.DEFAULT_TYPE):
   """Affiche les positions ouvertes avec PnL temps réel."""
   ...
   # Fetch prix live — une requête par token avec filtrage chain
...
async def cmd_resume(update: Update, context: ContextTypes.DEFAULT_TYPE):
   ...
   _paused = False
   await update.message.reply_text(
       "▶️ Bot <b>repris</b>. Scanning actif.",
       parse_mode=ParseMode.HTML,
   )
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
   text = (
       ...
       "/positions — Positions ouvertes + PnL live\n"
