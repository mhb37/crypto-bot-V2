...
async def _check_close_trades(current_prices: dict) -> list:
   """Délègue la vérification SL/TP/timeout au trader (async)."""
...
async def main():
   ...
   logger.info(
       "Trader: %s | Portfolio: $%.2f | Positions max: %d | TP: +30%% | SL: -10%% | Timeout: 24h",
   ...
   if tg_ok:
       tg.set_trader(_trader)  # ← donne accès au trader pour /positions
